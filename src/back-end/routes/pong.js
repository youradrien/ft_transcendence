...
const attachSocketHandler = async (socket, USER_ID, fastify) => {
  if (!socket) {
    return;
  }
  // players inputs
  socket.on('message', message => {
    try {
      ...
      if (data?.type == 'paddle_move') {
        ...
        // game exists?
        let _game = null;
        for (const [_r, game] of fastify.p_rooms.entries()) {
      ...
      if (data?.type == 'player_giveup') {
        ...
        if (_game != null) {
          handleGameEnd(_game, 'give-up', fastify, USER_ID);
        }
      }
    } catch (err) {
...
async function pong_routes(fastify, options) {
  ...
  fastify.get('/api/pong/status', { preValidation: [fastify.authenticate] }, async (request, reply) => {
    ...
    return reply.send({
      ...
      data: {
        ...
        joinedQueue: !!fastify.p_waitingPlayers.has(USER__ID),
        alr_in_game: alreadyInGame,
        is_ai: isAiGame,
      },
    });
  });
  ...
  fastify.get('/api/pong/active-games', { preValidation: [fastify.authenticate] }, async (request, reply) => {
    ...
    for (const [id, game] of fastify.p_rooms.entries()) {
      if (game.isAI === true) {
        const player = await fastify.db.get('SELECT id, username FROM users WHERE id = ?', [game.players[0]]);
        ...
        const players = await fastify.db.all('SELECT id, username FROM users WHERE id IN (?, ?)', [
          game.players[0],
          game.players[1],
        ]);
        // quick lookup
        const userMap = new Map(players.map(user => [user.id, user.username]));
        rooms.push({
          id,
          isAI: false, // 🔥 Add flag
          player1: {
            id: game.players[0],
            username: userMap.get(game.players[0]) || `/${game.players[0]}/`,
            score: game.scores.p1,
          },
          player2: {
            id: game.players[1],
            username: userMap.get(game.players[1]) || `/${game.players[1]}/`,
            score: game.scores.p2,
          },
        });
      }
  ...
  fastify.get('/api/pong/ws', { websocket: true }, async (connection, req) => {
    ...
    for (const [roomId, _g] of fastify.p_rooms.entries()) {
      if (Array.isArray(_g.players) && _g.players.includes(USER_ID)) {
        ...
        const playerIndex = _g.players.indexOf(USER_ID);
        if (playerIndex !== -1) {
          _g.sockets[playerIndex] = connection.socket; // <- important brr
          attachSocketHandler(connection.socket, USER_ID, fastify);
        }
        ...
        if (_g?.countdown <= 0) {
          setTimeout(() => {
            ...
            const sf = {
              ...
              width: _g.width,
              height: _g.height,
              paddleWidth: _g.paddleWidth,
              paddleHeight: _g.paddleHeight,
              max_score: _g.max_score,
    ...
    if (p_waitingPlayers.size >= 2) {
      ...
      const gameId = `${Date.now()}_${p1Id}_${p2Id}`;
      ...
      const playerNames = await fastify.db.all('SELECT id, username FROM users WHERE id IN (?, ?)', [p1Id, p2Id]);
      const nameMap = Object.fromEntries(playerNames.map(r => [r.id, r.username]));
      game.player_names = playerNames.map(obj => nameMap[obj.id] || null);
  ...
  fastify.get('/api/pong/ai/ws', { websocket: true }, async (connection, req) => {
      ...
      connection.socket.on('message', message => {
        try {
          ...
          if (data.type === 'player_giveup') {
            ...
            handleAiGameEnd(existingGame, 'give-up', fastify, USER_ID);
          }
        } catch (err) {
          console.error('Invalid message:', err);
        }
    ...
    // Crée une partie AI
    const gameId = `ai_${Date.now()}_${USER_ID}`;
    const AI_ID = 'AI_BOT';
    ...
    connection.socket.on('message', message => {
      try {
        ...
        if (data.type === 'player_giveup') {
          handleAiGameEnd(game, 'give-up', fastify, USER_ID);
        }
      } catch (err) {
        console.error('Invalid message:', err);
      }
    ...
    // Cleanup
    connection.socket.on('close', () => {
      console.log(`AI game ${gameId} ended`);
      if (game.interval) clearInterval(game.interval);
      p_rooms.delete(gameId);
    });
  });
}
...
const startGameLoop = (game, fastify = null) => {
  const interval = setInterval(() => {
    ...
    if (game.ball.x <= 0 || game.ball.x >= game.width) {
      if (game.scores.p1 >= game.max_score || game.scores.p2 >= game.max_score) {
        handleGameEnd(game, 'victory', fastify);
        return;
      }
    }
...
const startAiGameLoop = (game, fastify = null) => {
  const interval = setInterval(() => {
    // Ball physics
    game.ball.x += game.ball.vx;
    game.ball.y += game.ball.vy;
    ...
    if (Math.random() < 0.001) {
      // ~5% chance per second at 60fps
      game.paddles.p2 += Math.random() * 60 - 30; // Sudden wrong move
    }

    // Clamp AI paddle
    if (game.paddles.p2 < 0) game.paddles.p2 = 0;
    if (game.paddles.p2 > game.height - game.paddleHeight) game.paddles.p2 = game.height - game.paddleHeight;
    ...
    if (game.ball.x <= 0 || game.ball.x >= game.width) {
      if (game.scores.p1 >= game.max_score || game.scores.p2 >= game.max_score) {
        handleAiGameEnd(game, 'victory', fastify);
        return;
      }
    }
    ...
    // Bounce left/right
    if (game.ball.x <= 0 || game.ball.x >= game.width) {
      if (Math.abs(game.ball.vx) < 3.5) game.ball.vx *= -1.15;
      else game.ball.vx *= -1;
    }
...
const handleGameEnd = async (game, reason = 'victory', fastify = null, userId = null) => {
  if (!game) {
    return;
  }
  ...
  //write to DB ---
  const winnerId = winner === 'p1' ? players[0] : winner === 'p2' ? players[1] : null;
  try {
    await fastify.db.run(
      ...
      [players[0], players[1], winner === 'p1' ? 1 : 2, scores.p1, scores.p2, player_names[0], player_names[1]]
    );
    console.log(`game ${game.id} saved to DB`);

    // --- update users (wins/losses) ---
    if (winnerId) {
      await fastify.db.run(`UPDATE users SET wins = wins + 1 WHERE id = ?`, [winnerId]);
      const loserId = winnerId === players[0] ? players[1] : players[0];
      await fastify.db.run(`UPDATE users SET losses = losses + 1 WHERE id = ?`, [loserId]);
    }
  } catch (err) {
  ...
  sockets.forEach((socket, i) => {
    if (socket.readyState === 1) {
      ...
      const d = {
        ...
        you_are_winner: winner === playerKey,
      };
...
const handleAiGameEnd = async (game, reason = 'victory', fastify = null, userId = null) => {
  ...
  } else if (reason == 'give-up') winner = 'p2';
  if (!sockets || !Array.isArray(sockets)) return;
  ...
  try {
    ...
    try {
      ...
      if (aiUser) {
        ...
        const result = await fastify.db.run('INSERT INTO users (username, email, password) VALUES (?, ?, ?)', [
          'AI_BOT',
          'ai@bot.local',
          'no_password',
        ]);
    ...
    const _insertResult = await fastify.db.run(
      `INSERT INTO games (
                player1_id, player2_id, winner_id,
                player1_score, player2_score,
                p1_name, p2_name
            ) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [players[0], AI_USER_ID, winner === 'p1' ? 1 : 2, scores.p1, scores.p2, player_names[0], player_names[1]]
    );
    console.log(`✅ [AI_GAME_END] AI game ${game.id} saved to DB`);

    // Update wins/losses SEULEMENT pour le joueur humain
    if (winner === 'p1') await fastify.db.run(`UPDATE users SET wins = wins + 1 WHERE id = ?`, [playerId]);
    else await fastify.db.run(`UPDATE users SET losses = losses + 1 WHERE id = ?`, [playerId]);
  } catch (err) {
