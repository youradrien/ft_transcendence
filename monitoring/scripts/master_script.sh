...
if [ "$arg" == "status" ] || [ "$arg" == "elk-status" ]; then
	...
	curl -k -s -u elastic:elastic_password https://localhost:9200/_cluster/health?pretty || echo "❌ Elasticsearch is not ready"
	echo -e "\n=== kibana status ==="
	curl -k -s https://localhost:5601/api/status | grep -o '"level":"[^"]*"' | head -1 || echo "❌ Kibana not ready"
	echo -e "\n=== logstash pipelines ==="
	curl -s http://localhost:9600/_node/stats | grep -o '"main":{[^}]*}' || echo "❌ Logstash not ready"
	echo -e "\n=== Log collection ==="
	curl -k -s -u elastic:elastic_password "https://localhost:9200/ft_transcendence-*/_count" | grep -o '"count":[0-9]*' || echo "❌ No logs found"
fi
