...
for i in {1..30}; do
  if curl -k -s -u "$ES_USER:$ES_PASS" "$ES_URL/_security/_authenticate" > /dev/null 2>&1; then
    break
  fi
  sleep 1
done
...
RESP=$(curl -k -s -u "$ES_USER:$ES_PASS" -X POST "$ES_URL/_security/user/kibana_system/_password" \
