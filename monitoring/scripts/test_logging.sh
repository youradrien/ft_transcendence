...
ES_HEALTH=$(curl -s -u elastic:elastic_password http://localhost:9200/_cluster/health | grep -o '"status":"[^\"]*"' | cut -d'"' -f4)
...
RECENT_LOGS=$(curl -s -u elastic:elastic_password "http://localhost:9200/ft_transcendence-*/_count")
...
SAMPLE_LOG=$(curl -s -u elastic:elastic_password "http://localhost:9200/ft_transcendence-*/_search?size=1&sort=@timestamp:desc" | jq '.')
...
PARSE_FAILURES=$(curl -s -u elastic:elastic_password "http://localhost:9200/ft_transcendence-*/_count?q=tags:_jsonparsefailure" | grep -o '"count":[0-9]*' | cut -d':' -f2)
...
FIELDS=$(curl -s -u elastic:elastic_password "http://localhost:9200/ft_transcendence-*/_mapping" | jq -r '.. | .properties? // empty | keys[]' | sort -u)
...
BACKEND_LOGS=$(curl -s -u elastic:elastic_password "http://localhost:9200/ft_transcendence-*/_count?q=service.name:backend" | grep -o '"count":[0-9]*' | cut -d':' -f2)
...
for level in INFO WARN ERROR DEBUG; do
    COUNT=$(curl -s -u elastic:elastic_password "http://localhost:9200/ft_transcendence-*/_count?q=log_level:${level}" | grep -o '"count":[0-9]*' | cut -d':' -f2)
    if [ "$COUNT" -gt 0 ]; then
...
for status in 200 201 400 401 403 404 409 500; do
    COUNT=$(curl -s -u elastic:elastic_password "http://localhost:9200/ft_transcendence-*/_count?q=http_status:${status}" | grep -o '"count":[0-9]*' | cut -d':' -f2)
    if [ "$COUNT" -gt 0 ]; then
