...
INDICES=$(curl -s "${ES_URL}/_cat/indices/ft_transcendence-*?h=index" | tr -d '\r')
...
if [ -z "${INDICES}" ]; then
  ...
  while IFS= read -r index; do
    [ -z "$index" ] && continue
    CURRENT=$(curl -s "${ES_URL}/${index}/_settings?flat_settings=true" | grep -o 'index\.lifecycle\.name":"[^"]*' | awk -F'":"' '{print $2}' || true)
